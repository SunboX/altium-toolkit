...
/**
 * Builds a fake model-bounds connector with a source origin offset from the
 * resolved owner footprint center.
 * @returns {{ scene: object, documentModel: object }}
 */
function createModelBoundsPartCodeConnectorCase() {
    return {
        scene: {
            sourceFormat: 'altium',
            board: { centerX: 500, centerY: 250, thicknessMil: 80 },
            externalPlacements: [
                {
                    ...
                    projection: {
                        source: 'model-bounds',
                        boundsMil: { width: 190, depth: 197, height: 126 }
                    },
                    externalModel: {
                        origin: 'embedded',
                        name: '788641001.step',
        ...
        documentModel: {
            sourceFormat: 'altium',
            kind: 'pcb',
            fileName: 'model-bounds-owner-fake.PcbDoc',
            pcb: {
                components: [
                    {
                        designator: 'J5',
...
/**
 * Verifies model-bounds connector bodies are centered on the matched footprint
 * when owned mechanical pads define a reliable owner center.
 */
test('Altium 3D owner repair centers part-code model-bounds connectors', () => {
    const { scene, documentModel } = createModelBoundsPartCodeConnectorCase()
    const repaired = AltiumScene3dExternalPlacementAdapter.apply(
        scene,
        documentModel
    )
